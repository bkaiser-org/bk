...
import { addCircleOutline, arrowUpOutline, arrowDownOutline, bulbOutline, checkmarkOutline, closeOutline, createOutline, 
  ...
  imports: [ 
    ...
    IonItemSliding, IonItemOptions, IonItemOption, IonImg, IonAvatar
  ],
  template: `
<ion-header>
          ...
          <ion-row>
            ...
            <ion-col size="6" size-md="4">
              <ion-button fill="clear">
                <ion-label color="light"><strong>{{ '@input.title.label' | translate | async }}</strong></ion-label>
              </ion-button>
            </ion-col>
          </ion-row>
        </ion-grid>
    ...
    @if(reservations.length === 0) {
      ...
      @for(_reservation of reservations; track _reservation.bkey) {
        <ion-item-sliding #slidingItem>
          <ion-item>
            <ion-avatar slot="start" (click)="edit(_reservation.subjectKey, _reservation.subjectType)" class="ion-hide-md-down">
              <ion-img src="{{_reservation.subjectType + '.' + _reservation.subjectKey | avatar | async}}" alt="Avatar Logo" />
            </ion-avatar>
            <ion-label (click)="edit(_reservation.subjectKey, _reservation.subjectType)" class="ion-hide-md-down">
              {{_reservation.subjectName | fullName:_reservation.subjectName2}}
            </ion-label>
...
export class BoatHouseReservationListComponent extends BaseModelListComponent implements OnInit {
  public baseService = inject(BoatHouseReservationsService);

  public RS = ReservationStates;
  ...
  public async addReservation(): Promise<void> {
    await this.baseService.createNewReservationFromResource(BoatHouseKey);
  }

  public async editReservation(reservation: RelationshipModel, slidingItem?: IonItemSliding): Promise<void> {
    if (slidingItem) slidingItem.close();
    await this.baseService.editReservation(reservation);
  }

  public async endReservation(reservation: RelationshipModel, slidingItem?: IonItemSliding): Promise<void> {
    if (slidingItem) slidingItem.close();
    await this.baseService.endReservation(reservation);
  }

  public export(): void {
    this.baseService.exportBoathouseReservations();
  }
}
