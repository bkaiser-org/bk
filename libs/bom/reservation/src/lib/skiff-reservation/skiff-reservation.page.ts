...
import { newReservationFromBooker } from '../reservation.util';
import { convertFormToReservation, convertFormToResource, convertReservationToForm } from '../reservation-form.util';
import { AuthService } from '@bk/auth';
import { DataService } from '@bk/base';
import { SkiffReservationFormComponent } from './skiff-reservation.form';

@Component({
  selector: 'bk-skiff-reservation-page',
  standalone: true,
  imports: [
    TranslatePipe, AsyncPipe, 
    BkChangeConfirmationComponent, BkHeaderComponent, SkiffReservationFormComponent,
  ...
  `],
  template: `
      <bk-header title="{{ '@event.type.reservation.skiff.title' | translate | async }}" />
      @if(formCanBeSaved) {
        <bk-change-confirmation [showCancel]="true" (okClicked)="save()" (cancelClicked)="cancel()" />
      } 
      <ion-content>
        <ion-item lines="none">
          <ion-label>{{ '@event.type.reservation.skiff.subTitle' | translate | async }}</ion-label>
        </ion-item>
        <bk-skiff-reservation-form [vm]="vm" (changedData)="onDataChange($event)" (changedFormState)="onFormStateChange($event)" (confirmationChanged)="onConfirmationChange($event)" />
      </ion-content>
  `
})
export class BkSkiffReservationPageComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (_uid) {
      ...
      if (_booker) {
        this.reservation = newReservationFromBooker(_booker);
        this.vm = convertReservationToForm(this.reservation);
      }
    } else {
      die('BkSkiffReservationPageComponent.ngOnInit: _uid is undefined.');
    }
  }
  ...
  public async save() {
    ...
    const _resource = convertFormToResource(this.currentForm);
    const _resourceKey = await this.resourceService.createResource(_resource);
    _reservation.objectKey = _resourceKey ?? '';
    ...
    confirm(this.alertController, '@event.type.reservation.skiff.confirmation', false);
