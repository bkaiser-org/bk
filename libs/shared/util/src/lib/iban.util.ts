...
export function parseIban(value: string | undefined, allowQrIban = true): string {
    if (value === undefined || value === null) {
        console.warn('parseIban: undefined value; returning empty string');
        return '';
    } 
    ...
    if (!_iban) {
      console.warn('parseIban: _iban could not be converted into electronic format; returning empty string');
      return '';
    } 
    if (ibantools.isValidIBAN(_iban, { allowQRIBAN: allowQrIban }) === false) {
      console.warn('parseIban: <' + _iban + '> is not a valid iban number; returning empty string');
...
export function formatIban(value: string | undefined, ibanFormat = IbanFormat.Friendly, allowQrIban = true): string {
    ...
    if (!_friendlyIban) die('iban.util/formatIban: conversion into friendly format failed: ' + _iban); // this should not happen
