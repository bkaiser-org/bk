...
export function checkAhv(ahvn13value: number | string): boolean {
  ...
  if (_ahvn13value.length !== 13) {
      console.warn(`ahv.util.ahvn2string(${_ahvn13value}) must contain 13 digits.`);
      return false;
  }
  if (!_ahvn13value.startsWith('756')) {
    console.warn(`ahv.util.ahvn2string(${_ahvn13value}) must start with Swiss country code 756.`);
    return false;
  }
  ...
  if(_checksum === parseInt(_ahvn13value.substring(12, 13))) {
    ...
    console.warn(`ahv.util.ahvn2string(${_ahvn13value}) has invalid checksum.`);
    return false; 
  }
}
