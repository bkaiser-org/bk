...
--------------------------------------------------------------------------*/
export function getCountryName(countryCode: string, lang: string): string {
    return i18nIsoCountries.getName(countryCode, lang, {select: 'official'}) ?? '';
}
...
export function getWikipediaUrl(countryCode: string, lang: string): string {
    const _lang = lang.toLowerCase().trim();
    return `https://${_lang}.wikipedia.org/wiki/${getCountryName(countryCode, _lang)}`;
}
